...
static noxtls_return_t noxtls_ripemd160_round(noxtls_sha_ctx_t * ctx, const uint8_t * block)
{
    ...
    for(j = 0; j < RIPEMD160_WORDS_PER_BLOCK; j++)
        w[j] = (uint32_t)block[j * RIPEMD160_WORD_BYTES] |
               ((uint32_t)block[j * RIPEMD160_WORD_BYTES + 1] << 8) |
               ((uint32_t)block[j * RIPEMD160_WORD_BYTES + 2] << 16) |
               ((uint32_t)block[j * RIPEMD160_WORD_BYTES + 3] << 24);
