...
#else
#include "noxtls_mldsa_config.h"
#endif

#include "noxtls_params.h"
#include "noxtls_sys.h"
 ...
 * set are additionally prefixed with 44/65/87. See noxtls_mldsa_config.h.
 *
 * Example: If MLD_CONFIG_NAMESPACE_PREFIX is noxtls_mldsa, then
 * MLD_NAMESPACE_KL(keypair) becomes noxtls_mldsa44_keypair/
 * noxtls_mldsa65_keypair/noxtls_mldsa87_keypair.
 */
...
#include <string.h>
#define noxtls_mld_memcpy memcpy
#endif

#if !defined(MLD_CONFIG_CUSTOM_MEMSET)
#include <string.h>
#define noxtls_mld_memset memset
#endif
...
#define MLD_ALLOC(v, T, N, context) \
  MLD_ALIGN T noxtls_mld_alloc_##v[N];     \
  T *v = noxtls_mld_alloc_##v
...
#define MLD_FREE(v, T, N, context)                     \
  do                                                   \
  {                                                    \
    noxtls_mld_zeroize(noxtls_mld_alloc_##v, sizeof(noxtls_mld_alloc_##v)); \
    (v) = NULL;                                        \
  } while(0)
...
#define MLD_FREE(v, T, N, context)                                            \
  ...
  {                                                                           \
    if(v != NULL)                                                            \
    {                                                                         \
      noxtls_mld_zeroize(v, sizeof(T) * (N));                                        \
