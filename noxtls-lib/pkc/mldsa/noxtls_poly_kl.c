...
#define noxtls_mld_rej_eta MLD_ADD_PARAM_SET(noxtls_mld_rej_eta)
#define noxtls_mld_rej_eta_c MLD_ADD_PARAM_SET(noxtls_mld_rej_eta_c)
#define noxtls_mld_poly_decompose_c MLD_ADD_PARAM_SET(noxtls_mld_poly_decompose_c)
#define noxtls_mld_poly_use_hint_c MLD_ADD_PARAM_SET(noxtls_mld_poly_use_hint_c)
#define noxtls_mld_polyz_unpack_c MLD_ADD_PARAM_SET(noxtls_mld_polyz_unpack_c)
/* End of parameter set namespacing */


MLD_STATIC_TESTABLE
void noxtls_mld_poly_decompose_c(noxtls_mld_poly *a1, noxtls_mld_poly *a0)
__contract__(
...
{
  unsigned int i;
  noxtls_mld_assert_bound(a0->coeffs, MLDSA_N, 0, MLDSA_Q);
  for(i = 0; i < MLDSA_N; ++i)
  __loop__(
    assigns(i, memory_slice(a0, sizeof(noxtls_mld_poly)), memory_slice(a1, sizeof(noxtls_mld_poly)))
    invariant(i <= MLDSA_N)
  ...
  {
    noxtls_mld_decompose(&a0->coeffs[i], &a1->coeffs[i], a0->coeffs[i]);
  }

  noxtls_mld_assert_abs_bound(a0->coeffs, MLDSA_N, MLDSA_GAMMA2 + 1);
  noxtls_mld_assert_bound(a1->coeffs, MLDSA_N, 0, (MLDSA_Q - 1) / (2 * MLDSA_GAMMA2));
}

MLD_INTERNAL_API
void noxtls_mld_poly_decompose(noxtls_mld_poly *a1, noxtls_mld_poly *a0)
{
#if defined(MLD_USE_NATIVE_POLY_DECOMPOSE_88) && MLD_CONFIG_PARAMETER_SET == 44
  int ret;
  noxtls_mld_assert_bound(a0->coeffs, MLDSA_N, 0, MLDSA_Q);
  ret = noxtls_mld_poly_decompose_88_native(a1->coeffs, a0->coeffs);
  if(ret == MLD_NATIVE_FUNC_SUCCESS)
  {
    noxtls_mld_assert_abs_bound(a0->coeffs, MLDSA_N, MLDSA_GAMMA2 + 1);
    noxtls_mld_assert_bound(a1->coeffs, MLDSA_N, 0,
  ...
  int ret;
  noxtls_mld_assert_bound(a0->coeffs, MLDSA_N, 0, MLDSA_Q);
  ret = noxtls_mld_poly_decompose_32_native(a1->coeffs, a0->coeffs);
  if(ret == MLD_NATIVE_FUNC_SUCCESS)
  {
    noxtls_mld_assert_abs_bound(a0->coeffs, MLDSA_N, MLDSA_GAMMA2 + 1);
    noxtls_mld_assert_bound(a1->coeffs, MLDSA_N, 0,
  ...
  noxtls_mld_poly_decompose_c(a1, a0);
}

MLD_INTERNAL_API
unsigned int noxtls_mld_poly_make_hint(noxtls_mld_poly *h, const noxtls_mld_poly *a0,
                                const noxtls_mld_poly *a1)
{
  unsigned int i;
  unsigned int s = 0;
  ...
  {
    const unsigned int hint_bit = noxtls_mld_make_hint(a0->coeffs[i], a1->coeffs[i]);
    h->coeffs[i] = (int32_t)hint_bit;
    s += hint_bit;
  }

  noxtls_mld_assert(s <= MLDSA_N);
  noxtls_mld_assert_bound(h->coeffs, MLDSA_N, 0, 2);
  return s;
}

MLD_STATIC_TESTABLE void noxtls_mld_poly_use_hint_c(noxtls_mld_poly *b, const noxtls_mld_poly *a,
                                             const noxtls_mld_poly *h)
__contract__(
  requires(memory_no_alias(a,  sizeof(noxtls_mld_poly)))
  requires(memory_no_alias(b, sizeof(noxtls_mld_poly)))
  requires(memory_no_alias(h, sizeof(noxtls_mld_poly)))
  requires(array_bound(a->coeffs, 0, MLDSA_N, 0, MLDSA_Q))
  requires(array_bound(h->coeffs, 0, MLDSA_N, 0, 2))
  assigns(memory_slice(b, sizeof(noxtls_mld_poly)))
  ensures(array_bound(b->coeffs, 0, MLDSA_N, 0, (MLDSA_Q-1)/(2*MLDSA_GAMMA2)))
)
{
  unsigned int i;
  noxtls_mld_assert_bound(a->coeffs, MLDSA_N, 0, MLDSA_Q);
  noxtls_mld_assert_bound(h->coeffs, MLDSA_N, 0, 2);
  ...
  {
    b->coeffs[i] = noxtls_mld_use_hint(a->coeffs[i], h->coeffs[i]);
  }
  noxtls_mld_assert_bound(b->coeffs, MLDSA_N, 0, (MLDSA_Q - 1) / (2 * MLDSA_GAMMA2));
}

MLD_INTERNAL_API
void noxtls_mld_poly_use_hint(noxtls_mld_poly *b, const noxtls_mld_poly *a, const noxtls_mld_poly *h)
{
#if defined(MLD_USE_NATIVE_POLY_USE_HINT_88) && MLD_CONFIG_PARAMETER_SET == 44
  int ret;
  noxtls_mld_assert_bound(a->coeffs, MLDSA_N, 0, MLDSA_Q);
  noxtls_mld_assert_bound(h->coeffs, MLDSA_N, 0, 2);
  ret = noxtls_mld_poly_use_hint_88_native(b->coeffs, a->coeffs, h->coeffs);
  if(ret == MLD_NATIVE_FUNC_SUCCESS)
  {
    noxtls_mld_assert_bound(b->coeffs, MLDSA_N, 0, (MLDSA_Q - 1) / (2 * MLDSA_GAMMA2));
    return;
  ...
  noxtls_mld_assert_bound(a->coeffs, MLDSA_N, 0, MLDSA_Q);
  noxtls_mld_assert_bound(h->coeffs, MLDSA_N, 0, 2);
  ret = noxtls_mld_poly_use_hint_32_native(b->coeffs, a->coeffs, h->coeffs);
  if(ret == MLD_NATIVE_FUNC_SUCCESS)
  {
    noxtls_mld_assert_bound(b->coeffs, MLDSA_N, 0, (MLDSA_Q - 1) / (2 * MLDSA_GAMMA2));
    return;
          ...
          65 || MLD_CONFIG_PARAMETER_SET == 87) */
  noxtls_mld_poly_use_hint_c(b, a, h);
}

/*************************************************
 * Name:        noxtls_mld_rej_eta
 *
...
/* Reference: `noxtls_mld_rej_eta()` in the reference implementation @[REF].
...
MLD_STATIC_TESTABLE unsigned int noxtls_mld_rej_eta_c(int32_t *a, unsigned int target,
                                               unsigned int offset,
                                               const uint8_t *buf,
                                               unsigned int buflen)
__contract__(
...
)
{
  ...
  uint32_t t0;
  uint32_t t1;
  noxtls_mld_assert_abs_bound(a, offset, MLDSA_ETA + 1);
  ctr = offset;
  pos = 0;
...
#error "Invalid value of MLDSA_ETA"
  ...
  noxtls_mld_assert_abs_bound(a, ctr, MLDSA_ETA + 1);

  return ctr;
}

static unsigned int noxtls_mld_rej_eta(int32_t *a, unsigned int target,
...
)
{
  ...
  if(offset == 0)
  {
    ret = noxtls_mld_rej_uniform_eta2_native(a, target, buf, buflen);
    if(ret != MLD_NATIVE_FUNC_FALLBACK)
    {
      unsigned res = (unsigned)ret;
      noxtls_mld_assert_abs_bound(a, res, MLDSA_ETA + 1);
      return res;
    }
  }
  ...
  if(offset == 0)
  {
    ret = noxtls_mld_rej_uniform_eta4_native(a, target, buf, buflen);
    if(ret != MLD_NATIVE_FUNC_FALLBACK)
    {
      unsigned res = (unsigned)ret;
      noxtls_mld_assert_abs_bound(a, res, MLDSA_ETA + 1);
  ...
  return noxtls_mld_rej_eta_c(a, target, offset, buf, buflen);
}

#if !defined(MLD_CONFIG_SERIAL_FIPS202_ONLY)
MLD_INTERNAL_API
void noxtls_mld_poly_uniform_eta_4x(noxtls_mld_poly *r0, noxtls_mld_poly *r1, noxtls_mld_poly *r2,
                             noxtls_mld_poly *r3, const uint8_t seed[MLDSA_CRHBYTES],
                             ...
                             uint8_t nonce3)
{
  ...
  unsigned ctr[4];
  noxtls_mld_xof256_x4_ctx state;
  unsigned buflen;

  noxtls_mld_memcpy(extseed[0], seed, MLDSA_CRHBYTES);
  noxtls_mld_memcpy(extseed[1], seed, MLDSA_CRHBYTES);
  noxtls_mld_memcpy(extseed[2], seed, MLDSA_CRHBYTES);
  noxtls_mld_memcpy(extseed[3], seed, MLDSA_CRHBYTES);
  ...
  noxtls_mld_xof256_x4_init(&state);
  noxtls_mld_xof256_x4_absorb(&state, extseed, MLDSA_CRHBYTES + 2);
   ...
   */
  noxtls_mld_xof256_x4_squeezeblocks(buf, MLD_POLY_UNIFORM_ETA_NBLOCKS, &state);
  buflen = MLD_POLY_UNIFORM_ETA_NBLOCKS * MLD_STREAM256_BLOCKBYTES;

  ctr[0] = noxtls_mld_rej_eta(r0->coeffs, MLDSA_N, 0, buf[0], buflen);
  ctr[1] = noxtls_mld_rej_eta(r1->coeffs, MLDSA_N, 0, buf[1], buflen);
  ctr[2] = noxtls_mld_rej_eta(r2->coeffs, MLDSA_N, 0, buf[2], buflen);
  ctr[3] = noxtls_mld_rej_eta(r3->coeffs, MLDSA_N, 0, buf[3], buflen);
  ...
  while(ctr[0] < MLDSA_N || ctr[1] < MLDSA_N || ctr[2] < MLDSA_N ||
         ctr[3] < MLDSA_N)
  __loop__(
    assigns(ctr, state, memory_slice(r0, sizeof(noxtls_mld_poly)),
            memory_slice(r1, sizeof(noxtls_mld_poly)), memory_slice(r2, sizeof(noxtls_mld_poly)),
            memory_slice(r3, sizeof(noxtls_mld_poly)), object_whole(buf[0]),
  ...
  {
    noxtls_mld_xof256_x4_squeezeblocks(buf, 1, &state);
    ctr[0] = noxtls_mld_rej_eta(r0->coeffs, MLDSA_N, ctr[0], buf[0], buflen);
    ctr[1] = noxtls_mld_rej_eta(r1->coeffs, MLDSA_N, ctr[1], buf[1], buflen);
    ctr[2] = noxtls_mld_rej_eta(r2->coeffs, MLDSA_N, ctr[2], buf[2], buflen);
    ctr[3] = noxtls_mld_rej_eta(r3->coeffs, MLDSA_N, ctr[3], buf[3], buflen);
  }

  noxtls_mld_xof256_x4_release(&state);

  noxtls_mld_assert_abs_bound(r0->coeffs, MLDSA_N, MLDSA_ETA + 1);
  noxtls_mld_assert_abs_bound(r1->coeffs, MLDSA_N, MLDSA_ETA + 1);
  noxtls_mld_assert_abs_bound(r2->coeffs, MLDSA_N, MLDSA_ETA + 1);
  noxtls_mld_assert_abs_bound(r3->coeffs, MLDSA_N, MLDSA_ETA + 1);

  /* @[FIPS204, Section 3.6.3] Destruction of intermediate values. */
  noxtls_mld_zeroize(buf, sizeof(buf));
  noxtls_mld_zeroize(extseed, sizeof(extseed));
}
#else  /* !MLD_CONFIG_SERIAL_FIPS202_ONLY */

MLD_INTERNAL_API
void noxtls_mld_poly_uniform_eta(noxtls_mld_poly *r, const uint8_t seed[MLDSA_CRHBYTES],
                          uint8_t nonce)
{
  ...
  /* Tracks the number of coefficients we have already sampled */
  unsigned ctr;
  noxtls_mld_xof256_ctx state;
  unsigned buflen;

  noxtls_mld_memcpy(extseed, seed, MLDSA_CRHBYTES);
  extseed[MLDSA_CRHBYTES] = nonce;
  extseed[MLDSA_CRHBYTES + 1] = 0;

  noxtls_mld_xof256_init(&state);
  noxtls_mld_xof256_absorb_once(&state, extseed, MLDSA_CRHBYTES + 2);

  /*
   * Initially, squeeze heuristic number of MLD_POLY_UNIFORM_ETA_NBLOCKS.
   * This should generate the coefficients with high probability.
   */
  noxtls_mld_xof256_squeezeblocks(buf, MLD_POLY_UNIFORM_ETA_NBLOCKS, &state);
  buflen = MLD_POLY_UNIFORM_ETA_NBLOCKS * MLD_STREAM256_BLOCKBYTES;

  ctr = noxtls_mld_rej_eta(r->coeffs, MLDSA_N, 0, buf, buflen);
  ...
  __loop__(
    assigns(ctr, object_whole(&state),
      object_whole(buf), memory_slice(r, sizeof(noxtls_mld_poly)))
    invariant(ctr <= MLDSA_N)
    invariant(state.pos <= SHAKE256_RATE)
    invariant(array_abs_bound(r->coeffs, 0, ctr, MLDSA_ETA + 1)))
  {
    noxtls_mld_xof256_squeezeblocks(buf, 1, &state);
    ctr = noxtls_mld_rej_eta(r->coeffs, MLDSA_N, ctr, buf, buflen);
  }

  noxtls_mld_xof256_release(&state);

  noxtls_mld_assert_abs_bound(r->coeffs, MLDSA_N, MLDSA_ETA + 1);
...
MLD_INTERNAL_API
void noxtls_mld_poly_uniform_gamma1(noxtls_mld_poly *a, const uint8_t seed[MLDSA_CRHBYTES],
                             uint16_t nonce)
{
  MLD_ALIGN uint8_t
  ...
  noxtls_mld_xof256_squeezeblocks(buf, MLD_POLY_UNIFORM_GAMMA1_NBLOCKS, &state);
  noxtls_mld_polyz_unpack(a, buf);

  noxtls_mld_xof256_release(&state);

  noxtls_mld_assert_bound(a->coeffs, MLDSA_N, -(MLDSA_GAMMA1 - 1), MLDSA_GAMMA1 + 1);
...
#if !defined(MLD_CONFIG_SERIAL_FIPS202_ONLY)
MLD_INTERNAL_API
void noxtls_mld_poly_uniform_gamma1_4x(noxtls_mld_poly *r0, noxtls_mld_poly *r1, noxtls_mld_poly *r2,
                                noxtls_mld_poly *r3,
                                ...
                                uint16_t nonce2, uint16_t nonce3)
{
  ...
  noxtls_mld_xof256_x4_init(&state);
  noxtls_mld_xof256_x4_absorb(&state, extseed, MLDSA_CRHBYTES + 2);
  noxtls_mld_xof256_x4_squeezeblocks(buf, MLD_POLY_UNIFORM_GAMMA1_NBLOCKS, &state);

  noxtls_mld_polyz_unpack(r0, buf[0]);
  noxtls_mld_polyz_unpack(r1, buf[1]);
  noxtls_mld_polyz_unpack(r2, buf[2]);
  noxtls_mld_polyz_unpack(r3, buf[3]);
  noxtls_mld_xof256_x4_release(&state);

  noxtls_mld_assert_bound(r0->coeffs, MLDSA_N, -(MLDSA_GAMMA1 - 1), MLDSA_GAMMA1 + 1);
  noxtls_mld_assert_bound(r1->coeffs, MLDSA_N, -(MLDSA_GAMMA1 - 1), MLDSA_GAMMA1 + 1);
  noxtls_mld_assert_bound(r2->coeffs, MLDSA_N, -(MLDSA_GAMMA1 - 1), MLDSA_GAMMA1 + 1);
  noxtls_mld_assert_bound(r3->coeffs, MLDSA_N, -(MLDSA_GAMMA1 - 1), MLDSA_GAMMA1 + 1);
...
MLD_INTERNAL_API
void noxtls_mld_poly_challenge(noxtls_mld_poly *c, const uint8_t seed[MLDSA_CTILDEBYTES])
{
  unsigned int i;
  unsigned int j;
  unsigned int pos;
  ...
  noxtls_mld_shake256_absorb(&state, seed, MLDSA_CTILDEBYTES);
  noxtls_mld_shake256_finalize(&state);
  noxtls_mld_shake256_squeeze(buf, SHAKE256_RATE, &state);
  ...
  noxtls_mld_memset(c, 0, sizeof(noxtls_mld_poly));

  for(i = MLDSA_N - MLDSA_TAU; i < MLDSA_N; ++i)
  __loop__(
    assigns(i, j, object_whole(buf), state, pos, memory_slice(c, sizeof(noxtls_mld_poly)), signs)
  ...
  noxtls_mld_assert_bound(c->coeffs, MLDSA_N, -1, 2);
  noxtls_mld_shake256_release(&state);

  /* @[FIPS204, Section 3.6.3] Destruction of intermediate values. */
  noxtls_mld_zeroize(buf, sizeof(buf));
  noxtls_mld_zeroize(&signs, sizeof(signs));
}

MLD_INTERNAL_API
void noxtls_mld_polyeta_pack(uint8_t r[MLDSA_POLYETA_PACKEDBYTES], const noxtls_mld_poly *a)
{
  unsigned int i;
  uint8_t t[8];

  noxtls_mld_assert_abs_bound(a->coeffs, MLDSA_N, MLDSA_ETA + 1);
...
void noxtls_mld_polyeta_unpack(noxtls_mld_poly *r, const uint8_t a[MLDSA_POLYETA_PACKEDBYTES])
{
  unsigned int i;
...
#error "Invalid value of MLDSA_ETA"
  ...
  noxtls_mld_assert_bound(r->coeffs, MLDSA_N, MLD_POLYETA_UNPACK_LOWER_BOUND,
                   MLDSA_ETA + 1);
}


MLD_INTERNAL_API
void noxtls_mld_polyz_pack(uint8_t r[MLDSA_POLYZ_PACKEDBYTES], const noxtls_mld_poly *a)
{
  unsigned int i;
  uint32_t t[4];
...
MLD_STATIC_TESTABLE void noxtls_mld_polyz_unpack_c(
    noxtls_mld_poly *r, const uint8_t a[MLDSA_POLYZ_PACKEDBYTES])
__contract__(
...
#endif /* MLD_CONFIG_PARAMETER_SET != 44 */
  noxtls_mld_assert_bound(r->coeffs, MLDSA_N, -(MLDSA_GAMMA1 - 1), MLDSA_GAMMA1 + 1);
}

MLD_INTERNAL_API
void noxtls_mld_polyz_unpack(noxtls_mld_poly *r, const uint8_t a[MLDSA_POLYZ_PACKEDBYTES])
{
#if defined(MLD_USE_NATIVE_POLYZ_UNPACK_17) && MLD_CONFIG_PARAMETER_SET == 44
  int ret;
  ret = noxtls_mld_polyz_unpack_17_native(r->coeffs, a);
  if(ret == MLD_NATIVE_FUNC_SUCCESS)
  {
    noxtls_mld_assert_bound(r->coeffs, MLDSA_N, -(MLDSA_GAMMA1 - 1), MLDSA_GAMMA1 + 1);
    return;
    ...
    (MLD_CONFIG_PARAMETER_SET == 65 || MLD_CONFIG_PARAMETER_SET == 87)
  int ret;
  ret = noxtls_mld_polyz_unpack_19_native(r->coeffs, a);
  if(ret == MLD_NATIVE_FUNC_SUCCESS)
  {
    noxtls_mld_assert_bound(r->coeffs, MLDSA_N, -(MLDSA_GAMMA1 - 1), MLDSA_GAMMA1 + 1);
    return;
  ...
  noxtls_mld_polyz_unpack_c(r, a);
}

MLD_INTERNAL_API
void noxtls_mld_polyw1_pack(uint8_t r[MLDSA_POLYW1_PACKEDBYTES], const noxtls_mld_poly *a)
{
  unsigned int i;

  noxtls_mld_assert_bound(a->coeffs, MLDSA_N, 0, (MLDSA_Q - 1) / (2 * MLDSA_GAMMA2));
...
#undef noxtls_mld_rej_eta
#undef noxtls_mld_rej_eta_c
#undef noxtls_mld_poly_decompose_c
#undef noxtls_mld_poly_use_hint_c
#undef noxtls_mld_polyz_unpack_c
