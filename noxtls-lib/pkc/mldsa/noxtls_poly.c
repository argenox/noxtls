...
{
  unsigned int i;
  noxtls_mld_assert_bound(a->coeffs, MLDSA_N, INT32_MIN, MLD_REDUCE32_DOMAIN_MAX);
  ...
  __loop__(
    ...
    decreases(MLDSA_N - i))
  {
    a->coeffs[i] = noxtls_mld_reduce32(a->coeffs[i]);
  }

  noxtls_mld_assert_bound(a->coeffs, MLDSA_N, -MLD_REDUCE32_RANGE_MAX,
                   MLD_REDUCE32_RANGE_MAX);
}

MLD_STATIC_TESTABLE void noxtls_mld_poly_caddq_c(noxtls_mld_poly *a)
__contract__(
...
{
  unsigned int i;
  noxtls_mld_assert_abs_bound(a->coeffs, MLDSA_N, MLDSA_Q);
  ...
  {
    a->coeffs[i] = noxtls_mld_caddq(a->coeffs[i]);
  }
...
void noxtls_mld_poly_caddq(noxtls_mld_poly *a)
{
  ...
  int ret;
  noxtls_mld_assert_abs_bound(a->coeffs, MLDSA_N, MLDSA_Q);
  ret = noxtls_mld_poly_caddq_native(a->coeffs);
...
void noxtls_mld_poly_add(noxtls_mld_poly *r, const noxtls_mld_poly *b)
{
  ...
  for(i = 0; i < MLDSA_N; ++i)
  __loop__(
    assigns(i, memory_slice(r, sizeof(noxtls_mld_poly)))
    invariant(i <= MLDSA_N)
...
{
  unsigned int i;
  noxtls_mld_assert_abs_bound(b->coeffs, MLDSA_N, MLDSA_Q);
  noxtls_mld_assert_abs_bound(r->coeffs, MLDSA_N, MLDSA_Q);
  ...
  noxtls_mld_assert_bound(r->coeffs, MLDSA_N, INT32_MIN, MLD_REDUCE32_DOMAIN_MAX);
}
...
{
  unsigned int i;
  noxtls_mld_assert_bound(a->coeffs, MLDSA_N, 0, 1 << 10);
...
static MLD_INLINE int32_t noxtls_mld_fqmul(int32_t a, int32_t b)
__contract__(
  requires(b > -MLDSA_Q_HALF && b < MLDSA_Q_HALF)
  ensures(return_value > -MLDSA_Q && return_value < MLDSA_Q)
)
{
  /* Bounds: We argue in noxtls_mld_montgomery_reduce() that the reult
   ...
   *              < 2^31 * MLDSA_Q
   */
  return noxtls_mld_montgomery_reduce((int64_t)a * (int64_t)b);
}

/* noxtls_mld_ntt_butterfly_block()
 *
...
static MLD_INLINE void noxtls_mld_ntt_butterfly_block(int32_t r[MLDSA_N],
                                               const int32_t zeta,
  ...
  ensures(array_abs_bound(r, start + 2 * len, MLDSA_N, bound)))
{
  ...
  {
    int32_t t;
    t = noxtls_mld_fqmul(r[j + len], zeta);
    r[j + len] = r[j] - t;
    r[j] = r[j] + t;
  }
}

/* noxtls_mld_ntt_layer()
 *
...
/* Reference: Embedded in `ntt()` in the reference implementation @[REF]. */
static MLD_INLINE void noxtls_mld_ntt_layer(int32_t r[MLDSA_N], const unsigned layer)
__contract__(
  requires(memory_no_alias(r, sizeof(int32_t) * MLDSA_N))
  ...
  ensures(array_abs_bound(r, 0, MLDSA_N, (layer + 1) * MLDSA_Q)))
{
  ...
  {
    int32_t zeta = noxtls_zetas[k++];
    noxtls_mld_ntt_butterfly_block(r, zeta, start, len, layer * MLDSA_Q);
  }
}

MLD_STATIC_TESTABLE void noxtls_mld_poly_ntt_c(noxtls_mld_poly *a)
__contract__(
  requires(memory_no_alias(a, sizeof(noxtls_mld_poly)))
  requires(array_abs_bound(a->coeffs, 0, MLDSA_N, MLDSA_Q))
  assigns(memory_slice(a, sizeof(noxtls_mld_poly)))
...
)
{
  ...
  {
    noxtls_mld_ntt_layer(r, layer);
  }

  noxtls_mld_assert_abs_bound(a->coeffs, MLDSA_N, MLD_NTT_BOUND);
}
...
void noxtls_mld_poly_ntt(noxtls_mld_poly *a)
{
  ...
  int ret;
  noxtls_mld_assert_abs_bound(a->coeffs, MLDSA_N, MLDSA_Q);
  ret = noxtls_mld_ntt_native(a->coeffs);
  if(ret == MLD_NATIVE_FUNC_SUCCESS)
  {
    noxtls_mld_assert_abs_bound(a->coeffs, MLDSA_N, MLD_NTT_BOUND);
    return;
  ...
  noxtls_mld_poly_ntt_c(a);
}

/*************************************************
 * Name:        noxtls_mld_fqscale
 *
 ...
 **************************************************/
static MLD_INLINE int32_t noxtls_mld_fqscale(int32_t a)
__contract__(
...
)
{
   ...
   * a special case of that in noxtls_mld_fqmul(). */
  return noxtls_mld_montgomery_reduce((int64_t)a * f);
}

/* Reference: Embedded into `invntt_tomont()` in the reference implementation
 * @[REF] */
static MLD_INLINE void noxtls_mld_invntt_layer(int32_t r[MLDSA_N], unsigned layer)
__contract__(
  requires(memory_no_alias(r, sizeof(int32_t) * MLDSA_N))
  requires(1 <= layer && layer <= 8)
  ...
  ensures(array_abs_bound(r, 0, MLDSA_N, (MLDSA_N >> (layer - 1)) * MLDSA_Q)))
{
    ...
    decreases(MLDSA_N - start))
  {
      ...
      decreases(start + len - j))
    {
      ...
      r[j] = t + r[j + len];
      r[j + len] = t - r[j + len];
      r[j + len] = noxtls_mld_fqmul(r[j + len], zeta);
    }
  }
}

MLD_STATIC_TESTABLE void noxtls_mld_poly_invntt_tomont_c(noxtls_mld_poly *a)
__contract__(
  requires(memory_no_alias(a, sizeof(noxtls_mld_poly)))
  requires(array_abs_bound(a->coeffs, 0, MLDSA_N, MLDSA_Q))
  assigns(memory_slice(a, sizeof(noxtls_mld_poly)))
...
)
{
  ...
  __loop__(
    ...
    decreases(layer))
  {
    noxtls_mld_invntt_layer(r, layer);
  }
  ...
  {
    r[j] = noxtls_mld_fqscale(r[j]);
  }

  noxtls_mld_assert_abs_bound(a->coeffs, MLDSA_N, MLD_INTT_BOUND);
}
...
void noxtls_mld_poly_invntt_tomont(noxtls_mld_poly *a)
{
  ...
  int ret;
  noxtls_mld_assert_abs_bound(a->coeffs, MLDSA_N, MLDSA_Q);
  ret = noxtls_mld_intt_native(a->coeffs);
  if(ret == MLD_NATIVE_FUNC_SUCCESS)
  {
    noxtls_mld_assert_abs_bound(a->coeffs, MLDSA_N, MLD_INTT_BOUND);
    return;
  ...
  noxtls_mld_poly_invntt_tomont_c(a);
}

MLD_STATIC_TESTABLE void noxtls_mld_poly_pointwise_montgomery_c(noxtls_mld_poly *c,
                                                         const noxtls_mld_poly *a,
...
)
{
  unsigned int i;
  noxtls_mld_assert_abs_bound(a->coeffs, MLDSA_N, MLD_NTT_BOUND);
  noxtls_mld_assert_abs_bound(b->coeffs, MLDSA_N, MLD_NTT_BOUND);
  ...
  {
    c->coeffs[i] = noxtls_mld_montgomery_reduce((int64_t)a->coeffs[i] * b->coeffs[i]);
  }
  noxtls_mld_assert_abs_bound(c->coeffs, MLDSA_N, MLDSA_Q);
}
                                   ...
                                   const noxtls_mld_poly *b)
{
  ...
  noxtls_mld_assert_abs_bound(a->coeffs, MLDSA_N, MLD_NTT_BOUND);
  noxtls_mld_assert_abs_bound(b->coeffs, MLDSA_N, MLD_NTT_BOUND);
  ret = noxtls_mld_poly_pointwise_montgomery_native(c->coeffs, a->coeffs, b->coeffs);
  if(ret == MLD_NATIVE_FUNC_SUCCESS)
  {
    noxtls_mld_assert_abs_bound(c->coeffs, MLDSA_N, MLDSA_Q);
    return;
  ...
  noxtls_mld_poly_pointwise_montgomery_c(c, a, b);
}
...
void noxtls_mld_poly_power2round(noxtls_mld_poly *a1, noxtls_mld_poly *a0, const noxtls_mld_poly *a)
{
  ...
  {
    noxtls_mld_power2round(&a0->coeffs[i], &a1->coeffs[i], a->coeffs[i]);
  }

  noxtls_mld_assert_bound(a0->coeffs, MLDSA_N, -(MLD_2_POW_D / 2) + 1,
                   (MLD_2_POW_D / 2) + 1);
  noxtls_mld_assert_bound(a1->coeffs, MLDSA_N, 0, ((MLDSA_Q - 1) / MLD_2_POW_D) + 1);
}
...
#endif
/* Reference: `noxtls_mld_rej_uniform()` in the reference implementation @[REF].
...
MLD_STATIC_TESTABLE unsigned int noxtls_mld_rej_uniform_c(int32_t *a,
                                                   unsigned int target,
                                                   unsigned int offset,
                                                   const uint8_t *buf,
                                                   unsigned int buflen)
...
)
{
  ...
  unsigned int pos;
  uint32_t t;
  noxtls_mld_assert_bound(a, offset, 0, MLDSA_Q);
  ...
  noxtls_mld_assert_bound(a, ctr, 0, MLDSA_Q);

  return ctr;
}
/*************************************************
 * Name:        noxtls_mld_rej_uniform
 *
 ...
 *              target buffer. This avoids shifting the buffer base in the
 *              caller, which appears tricky to reason about. */
static unsigned int noxtls_mld_rej_uniform(int32_t *a, unsigned int target,
...
)
{
  ...
  if(offset == 0)
  {
    ret = noxtls_mld_rej_uniform_native(a, target, buf, buflen);
    if(ret != MLD_NATIVE_FUNC_FALLBACK)
    {
      unsigned res = (unsigned)ret;
      noxtls_mld_assert_bound(a, res, 0, MLDSA_Q);
      return res;
    }
  }
#endif /* MLD_USE_NATIVE_REJ_UNIFORM */

  return noxtls_mld_rej_uniform_c(a, target, offset, buf, buflen);
}
...
void noxtls_mld_poly_uniform(noxtls_mld_poly *a, const uint8_t seed[MLDSA_SEEDBYTES + 2])
{
  ...
  noxtls_mld_xof128_ctx state;

  noxtls_mld_xof128_init(&state);
  noxtls_mld_xof128_absorb_once(&state, seed, MLDSA_SEEDBYTES + 2);
  noxtls_mld_xof128_squeezeblocks(buf, MLD_POLY_UNIFORM_NBLOCKS, &state);

  ctr = noxtls_mld_rej_uniform(a->coeffs, MLDSA_N, 0, buf, buflen);
  buflen = MLD_STREAM128_BLOCKBYTES;
  while(ctr < MLDSA_N)
  __loop__(
    assigns(ctr, state, memory_slice(a, sizeof(noxtls_mld_poly)), object_whole(buf))
    invariant(ctr <= MLDSA_N)
    invariant(array_bound(a->coeffs, 0, ctr, 0, MLDSA_Q))
    invariant(state.pos <= SHAKE128_RATE)
  )
  {
    noxtls_mld_xof128_squeezeblocks(buf, 1, &state);
    ctr = noxtls_mld_rej_uniform(a->coeffs, MLDSA_N, ctr, buf, buflen);
  }
  noxtls_mld_xof128_release(&state);
  noxtls_mld_assert_bound(a->coeffs, MLDSA_N, 0, MLDSA_Q);
                         ...
                         uint8_t seed[4][MLD_ALIGN_UP(MLDSA_SEEDBYTES + 2)])
{
  ...
  unsigned ctr[4];
  noxtls_mld_xof128_x4_ctx state;
  unsigned buflen;

  noxtls_mld_xof128_x4_init(&state);
  noxtls_mld_xof128_x4_absorb(&state, seed, MLDSA_SEEDBYTES + 2);
  ...
  noxtls_mld_xof128_x4_squeezeblocks(buf, MLD_POLY_UNIFORM_NBLOCKS, &state);
  buflen = MLD_POLY_UNIFORM_NBLOCKS * MLD_STREAM128_BLOCKBYTES;
  ctr[0] = noxtls_mld_rej_uniform(vec0->coeffs, MLDSA_N, 0, buf[0], buflen);
  ctr[1] = noxtls_mld_rej_uniform(vec1->coeffs, MLDSA_N, 0, buf[1], buflen);
  ctr[2] = noxtls_mld_rej_uniform(vec2->coeffs, MLDSA_N, 0, buf[2], buflen);
  ctr[3] = noxtls_mld_rej_uniform(vec3->coeffs, MLDSA_N, 0, buf[3], buflen);
  ...
  __loop__(
    assigns(ctr, state, object_whole(buf),
            memory_slice(vec0, sizeof(noxtls_mld_poly)), memory_slice(vec1, sizeof(noxtls_mld_poly)),
            memory_slice(vec2, sizeof(noxtls_mld_poly)), memory_slice(vec3, sizeof(noxtls_mld_poly)))
  ...
  {
    noxtls_mld_xof128_x4_squeezeblocks(buf, 1, &state);
    ctr[0] = noxtls_mld_rej_uniform(vec0->coeffs, MLDSA_N, ctr[0], buf[0], buflen);
    ctr[1] = noxtls_mld_rej_uniform(vec1->coeffs, MLDSA_N, ctr[1], buf[1], buflen);
    ctr[2] = noxtls_mld_rej_uniform(vec2->coeffs, MLDSA_N, ctr[2], buf[2], buflen);
    ctr[3] = noxtls_mld_rej_uniform(vec3->coeffs, MLDSA_N, ctr[3], buf[3], buflen);
  }
  noxtls_mld_xof128_x4_release(&state);

  noxtls_mld_assert_bound(vec0->coeffs, MLDSA_N, 0, MLDSA_Q);
  noxtls_mld_assert_bound(vec1->coeffs, MLDSA_N, 0, MLDSA_Q);
  noxtls_mld_assert_bound(vec2->coeffs, MLDSA_N, 0, MLDSA_Q);
  noxtls_mld_assert_bound(vec3->coeffs, MLDSA_N, 0, MLDSA_Q);
...
void noxtls_mld_polyt1_unpack(noxtls_mld_poly *r, const uint8_t a[MLDSA_POLYT1_PACKEDBYTES])
{
  ...
  noxtls_mld_assert_bound(r->coeffs, MLDSA_N, 0, 1 << 10);
}
...
void noxtls_mld_polyt0_pack(uint8_t r[MLDSA_POLYT0_PACKEDBYTES], const noxtls_mld_poly *a)
{
  ...
  noxtls_mld_assert_bound(a->coeffs, MLDSA_N, -(1 << (MLDSA_D - 1)) + 1,
                   (1 << (MLDSA_D - 1)) + 1);
...
void noxtls_mld_polyt0_unpack(noxtls_mld_poly *r, const uint8_t a[MLDSA_POLYT0_PACKEDBYTES])
{
  ...
  noxtls_mld_assert_bound(r->coeffs, MLDSA_N, -(1 << (MLDSA_D - 1)) + 1,
                   (1 << (MLDSA_D - 1)) + 1);
}

MLD_STATIC_TESTABLE uint32_t noxtls_mld_poly_chknorm_c(const noxtls_mld_poly *a, int32_t B)
__contract__(
  requires(memory_no_alias(a, sizeof(noxtls_mld_poly)))
...
)
{
  ...
  )
  {
    ...
    /* if(abs(a[i]) >= B) */
    t |= noxtls_mld_ct_cmask_neg_i32(B - 1 - noxtls_mld_ct_abs_i32(a->coeffs[i]));
  }
...
uint32_t noxtls_mld_poly_chknorm(const noxtls_mld_poly *a, int32_t B)
{
   ...
   */
  ret = noxtls_mld_poly_chknorm_native(a->coeffs, B);
  ...
  if(success)
  {
    ...
    return noxtls_mld_ct_cmask_nonzero_u32((uint32_t)ret);
  }
#endif /* MLD_USE_NATIVE_POLY_CHKNORM */
  return noxtls_mld_poly_chknorm_c(a, B);
}

#else  /* !MLD_CONFIG_MULTILEVEL_NO_SHARED */
MLD_EMPTY_CU(noxtls_mld_poly)
