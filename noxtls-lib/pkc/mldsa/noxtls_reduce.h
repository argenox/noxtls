...
static MLD_INLINE int32_t noxtls_mld_montgomery_reduce(int64_t a)
__contract__(
...
)
{
  ...
  const uint32_t a_reduced = noxtls_mld_cast_int64_to_uint32(a);
  const uint32_t a_inverted = (a_reduced * QINV) & UINT32_MAX;

  /* Lift to signed canonical representative mod 2^32. */
  const int32_t t = noxtls_mld_cast_uint32_to_int32(a_inverted);

  int64_t r;

  noxtls_mld_assert(a < +(INT64_MAX - (((int64_t)1 << 31) * MLDSA_Q)) &&
 ...
 **************************************************/
MLD_MUST_CHECK_RETURN_VALUE
static MLD_INLINE int32_t noxtls_mld_reduce32(int32_t a)
__contract__(
...
)
{
  ...
  t = (a + (1 << 22)) >> 23;
  t = a - t * MLDSA_Q;
  noxtls_mld_assert((t - a) % MLDSA_Q == 0);
  return t;
}

/*************************************************
 * Name:        noxtls_mld_caddq
 *
 ...
 **************************************************/
MLD_MUST_CHECK_RETURN_VALUE
static MLD_INLINE int32_t noxtls_mld_caddq(int32_t a)
__contract__(
  requires(a > -MLDSA_Q)
  requires(a < MLDSA_Q)
...
{
  return noxtls_mld_ct_sel_int32(a + MLDSA_Q, a, noxtls_mld_ct_cmask_neg_i32(a));
}
