...
#define noxtls_mld_power2round MLD_ADD_PARAM_SET(noxtls_mld_power2round)
#define noxtls_mld_decompose MLD_ADD_PARAM_SET(noxtls_mld_decompose)
#define noxtls_mld_make_hint MLD_ADD_PARAM_SET(noxtls_mld_make_hint)
#define noxtls_mld_use_hint MLD_ADD_PARAM_SET(noxtls_mld_use_hint)
/* End of parameter set namespacing */

#define MLD_2_POW_D (1 << MLDSA_D)

/*************************************************
 * Name:        noxtls_mld_power2round
 *
 ...
 **************************************************/
static MLD_INLINE void noxtls_mld_power2round(int32_t *a0, int32_t *a1, int32_t a)
__contract__(
 ...
 **************************************************/
static MLD_INLINE void noxtls_mld_decompose(int32_t *a0, int32_t *a1, int32_t a)
__contract__(
  requires(memory_no_alias(a0, sizeof(int32_t)))
  requires(memory_no_alias(a1, sizeof(int32_t)))
  requires(a >= 0 && a < MLDSA_Q)
...
)
{
  ...
  *a1 = (a + 127) >> 7;
  ...
  noxtls_mld_assert(*a1 >= 0 && *a1 <= 65472);
  ...
  /*
   * Compute f1 = round-(f1' / B) Ã¢â€°Ë† round(f1' * 11275 / 2^24). This is exact for
   * 0 <= f1' < 2^16.
   *
   * To see this, consider the (signed) error f1' * (1 / B - 11275 / 2^24)
   * between f1' / B and the (under-)approximation f1' * 11275 / 2^24. Because
   * eps := 1 / B - 11275 / 2^24 is 1 / 1560281088 Ã¢â€°Ë† 2^(-30.54) < 2^(-30), we
   ...
   */
  *a1 = (*a1 * 11275 + (1 << 23)) >> 24;
  noxtls_mld_assert(*a1 >= 0 && *a1 <= 44);

  *a1 = noxtls_mld_ct_sel_int32(0, *a1, noxtls_mld_ct_cmask_neg_i32(43 - *a1));
  noxtls_mld_assert(*a1 >= 0 && *a1 <= 43);
  ...
  /*
   * Compute f1 = round-(f1' / B) Ã¢â€°Ë† round(f1' * 1025 / 2^22). This is exact for
   * 0 <= f1' < 2^16. Following the same argument above, it suffices to show
   * that f1' * eps < 1 / B, where eps := 1 / B - 1025 / 2^22. Indeed, we have
   * eps = 1 / 4290772992 Ã¢â€°Ë† 2^(-31.99) < 2^(-31), therefore f1' * eps <
   * 2^16 * 2^(-31) = 1 / 2^15 < 1 / 2^12 < 1 / B.
   */
  *a1 = (*a1 * 1025 + (1 << 21)) >> 22;
  noxtls_mld_assert(*a1 >= 0 && *a1 <= 16);

  *a1 &= 15;
  noxtls_mld_assert(*a1 >= 0 && *a1 <= 15);

#endif /* MLD_CONFIG_PARAMETER_SET != 44 */

  *a0 = a - *a1 * 2 * MLDSA_GAMMA2;
  *a0 = noxtls_mld_ct_sel_int32(*a0 - MLDSA_Q, *a0,
                         noxtls_mld_ct_cmask_neg_i32((MLDSA_Q - 1) / 2 - *a0));
}

/*************************************************
 * Name:        noxtls_mld_make_hint
 *
...
static MLD_INLINE unsigned int noxtls_mld_make_hint(int32_t a0, int32_t a1)
__contract__(
 ...
 **************************************************/
MLD_MUST_CHECK_RETURN_VALUE
static MLD_INLINE int32_t noxtls_mld_use_hint(int32_t a, int32_t hint)
__contract__(
