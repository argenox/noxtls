...
* Summary: STM32F4 AES acceleration backend (HAL CRYP).
*****************************************************************************/

#include "vendor/st/noxtls_stm32_accel.h"
#include <string.h>

#if defined(NOXTLS_STM32_FAMILY_F4) && defined(__has_include)
#if __has_include("stm32f4xx_hal.h")
#include "stm32f4xx_hal.h"
#endif
#if __has_include("stm32f4xx_hal_cryp.h")
#include "stm32f4xx_hal_cryp.h"
#endif
#endif

#if defined(NOXTLS_STM32_FAMILY_F4) && defined(__STM32F4xx_HAL_CRYP_H) && \
    defined(HAL_CRYP_MODULE_ENABLED) && (defined(CRYP) || defined(AES))
#define NOXTLS_STM32F4_HAL_CRYP_AVAILABLE 1
#else
#define NOXTLS_STM32F4_HAL_CRYP_AVAILABLE 0
#endif

#define NOXTLS_STM32F4_AES_BLOCK_WORDS 4u
#define NOXTLS_STM32F4_CRYP_TIMEOUT_MS 1000u

#if NOXTLS_STM32F4_HAL_CRYP_AVAILABLE
static noxtls_return_t noxtls_stm32f4_map_hal_status(HAL_StatusTypeDef status)
{
    switch(status) {
        case HAL_OK:
...
static noxtls_return_t noxtls_stm32f4_get_key_size(noxtls_aes_type_t type,
...
static noxtls_return_t noxtls_stm32f4_process_block(const uint8_t *key,
                                                     const uint8_t *data,
                                                     uint8_t *output,
                                                     ...
                                                     int encrypt)
{
    ...
    uint32_t input_words[NOXTLS_STM32F4_AES_BLOCK_WORDS] = {0u};
    uint32_t output_words[NOXTLS_STM32F4_AES_BLOCK_WORDS] = {0u};
    uint16_t size_arg = (uint16_t)NOXTLS_STM32F4_AES_BLOCK_WORDS;
    noxtls_return_t rc;
    ...
    rc = noxtls_stm32f4_get_key_size(type, &hal_key_size, &key_bytes);
    ...
    if(hal_status != HAL_OK) {
        return noxtls_stm32f4_map_hal_status(hal_status);
    }
    ...
    if(encrypt != 0) {
        hal_status = HAL_CRYP_Encrypt(&hcryp,
                                      ...
                                      size_arg,
                                      output_words,
                                      NOXTLS_STM32F4_CRYP_TIMEOUT_MS);
    } else {
...
#endif /* NOXTLS_STM32F4_HAL_CRYP_AVAILABLE */
...
{
#if NOXTLS_STM32F4_HAL_CRYP_AVAILABLE
    return noxtls_stm32f4_process_block(key, data, output, type, 1);
#else
    (void)key;
    (void)data;
    (void)output;
...
{
#if NOXTLS_STM32F4_HAL_CRYP_AVAILABLE
    return noxtls_stm32f4_process_block(key, data, output, type, 0);
#else
    (void)key;
    (void)data;
    (void)output;
