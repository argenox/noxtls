...
**Parameters:** `key` — encryption key; `data` — plaintext; `data_len` — length (multiple of 16); `iv` — unused (NULL); `output` — ciphertext buffer; `type` — `NOXTLS_ARIA_128_BIT`, `NOXTLS_ARIA_192_BIT`, or `NOXTLS_ARIA_256_BIT`.
