...
For a caller-polled TLS connection, select `TLS_IO_MODE_NON_BLOCKING`, set a
...
re-encrypting application data. For a version-specific TLS 1.2 integration,
use `noxtls_tls12_connect_poll()` or `noxtls_tls12_accept_poll()`.
