...
sidebar_position: 14
title: TLS (common)
description: "NoxTLS TLS (common) C API reference for embedded TLS, DTLS, and cryptography."
---
...
In caller-polled mode, return `TLS_IO_WOULD_BLOCK` (`-2`) when the transport is
temporarily unable to accept more bytes. Short positive writes are supported.
...
In caller-polled mode, return `TLS_IO_WOULD_BLOCK` (`-2`) when no input is
currently available. A zero return continues to mean end of stream.
...
### `noxtls_tls_set_io_mode`

```c
noxtls_return_t noxtls_tls_set_io_mode(tls_context_t *ctx, tls_io_mode_t mode);
```

Select `TLS_IO_MODE_BLOCKING` (default) or `TLS_IO_MODE_NON_BLOCKING`. In
nonblocking mode, record operations return `NOXTLS_RETURN_WANT_READ` or
`NOXTLS_RETURN_WANT_WRITE` when they need application-driven I/O progress.

### `noxtls_tls_set_io_tx_queue_limit`

```c
noxtls_return_t noxtls_tls_set_io_tx_queue_limit(tls_context_t *ctx, uint32_t limit);
```

Set the maximum encrypted bytes retained across partial writes. The default is
`NOXTLS_TLS_IO_TX_QUEUE_LIMIT` (256 KiB). Choose a bounded value appropriate
for the connection's memory budget.

### `noxtls_tls_flush` and `noxtls_tls_has_pending_output`
...
Call `noxtls_tls_flush()` when the transport becomes writable. It flushes
already-encrypted bytes and never re-encrypts queued application data.
`noxtls_tls_has_pending_output()` reports whether a later writable event still
needs to be handled.
