...
### `noxtls_tls_connection_set_io_mode`

```c
noxtls_return_t noxtls_tls_connection_set_io_mode(
    noxtls_tls_connection_t *conn, tls_io_mode_t mode);
```

Select blocking (default) or caller-polled nonblocking operation. In
nonblocking mode, the handshake and record APIs can return
`NOXTLS_RETURN_WANT_READ` or `NOXTLS_RETURN_WANT_WRITE`.

### `noxtls_tls_connection_set_io_tx_queue_limit`
...
Set the bounded encrypted-output queue limit before the handshake starts.

### `noxtls_tls_connection_flush`

```c
noxtls_return_t noxtls_tls_connection_flush(noxtls_tls_connection_t *conn);
```

Flush retained encrypted output after the transport reports writability.
...
### `noxtls_tls_connection_set_verify_crl`

```c
noxtls_return_t noxtls_tls_connection_set_verify_crl(
    noxtls_tls_connection_t *conn, const noxtls_x509_crl_t *crl);
```

Apply an application-owned CRL chain to certificate verification for either
negotiated protocol version. Keep the CRL available for the connection
lifetime.

### `noxtls_tls_connection_set_maximum_record_payload`
...
Request a bounded plaintext record payload before the handshake. TLS 1.2
accepts the RFC 6066 sizes 512, 1024, 2048, or 4096; TLS 1.3 uses RFC 8449.
Pass zero to restore the protocol default.

### `noxtls_tls_connection_set_tls13_session`
...
Copy caller-owned TLS 1.3 resumption state to apply when a client context is
created. See [TLS 1.3 session export/import](./tls13#session-resumption-portability).
...
### `noxtls_tls_connection_handshake`

```c
noxtls_return_t noxtls_tls_connection_handshake(noxtls_tls_connection_t *conn);
```

Advance a client or server handshake until it completes, needs input, needs an
output flush, or reaches a terminal error. Use this API for nonblocking
connections after the connection has been initialized and configured.
...
uint16_t noxtls_tls_connection_get_cipher_suite(
    const noxtls_tls_connection_t *conn);
...
These accessors expose connection metadata after negotiation. Certificate and
identity pointers are borrowed and remain valid only for the connection
lifetime. Session identities are opaque cache keys, not application identities.
