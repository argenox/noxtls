...
### `noxtls_tls12_connect_poll`

```c
noxtls_return_t noxtls_tls12_connect_poll(tls12_context_t *ctx);
```

Advance an initial client handshake when the base context uses
`TLS_IO_MODE_NON_BLOCKING`. Repeatedly call it until it returns success or a
terminal error. `NOXTLS_RETURN_WANT_READ` and `NOXTLS_RETURN_WANT_WRITE` are
resumable outcomes; flush pending output when the transport is writable.
...
### `noxtls_tls12_accept_poll`

```c
noxtls_return_t noxtls_tls12_accept_poll(tls12_context_t *ctx);
```

Server equivalent of `noxtls_tls12_connect_poll()`. Configure I/O and server
credentials before the first call.
...
### `noxtls_tls12_require_client_auth`

```c
void noxtls_tls12_require_client_auth(tls12_context_t *ctx, int require);
```

Server: require a non-empty client certificate. A nonzero value implies
`noxtls_tls12_request_client_auth()` and fails the handshake if the client
responds with an empty Certificate message.
