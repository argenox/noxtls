...
TLS 1.3 context: base DTLS/TLS context plus handshake state, key derivation (early/handshake/master and traffic secrets), keys/IVs and record number encryption keys, certificate fields, handshake buffer, key shares, extensions, SNI, optional server RSA or crypto provider, client auth (cert, keys, provider handle), external PSK and session ticket/resumption state, 0-RTT early data state, Connection ID (RFC 9147), workspace buffers, channel binding, and record size limit. Initialized with [noxtls_tls13_context_init](#noxtls_tls13_context_init), freed with [noxtls_tls13_context_free](#noxtls_tls13_context_free).

### noxtls_tls13_session_t

Caller-owned TLS 1.3 resumption state: ticket identity and nonce, resumption
PSK, ticket age-add, and cipher suite. Treat this structure as secret material;
protect it at rest and erase it when it is no longer needed.
...
### Session resumption portability

### `noxtls_tls13_session_export` and `noxtls_tls13_session_import`
...
Export a client ticket after it has been received, then import it into a new
client context before ClientHello is sent. The application owns storage and
cache lifetime. Bind stored state to the intended peer and TLS policy; the
opaque ticket identity is not an authentication identity by itself.

### Handshake and data

### `noxtls_tls13_connect`
...
### `noxtls_tls13_set_server_private_rsa`
...
### `noxtls_tls13_set_server_private_mldsa`
...
### `noxtls_tls13_request_client_auth`

```c
void noxtls_tls13_request_client_auth(tls13_context_t *ctx, int request);
```

Server: request client certificate (send CertificateRequest). Call before [noxtls_tls13_accept](#noxtls_tls13_accept).

### `noxtls_tls13_set_client_cert`
...
### `noxtls_tls13_set_prefer_chacha20`
...
### `noxtls_tls13_set_record_size_limit`
...
- **noxtls_tls13_send_client_hello**, **noxtls_tls13_recv_server_hello**, **noxtls_tls13_recv_encrypted_extensions**, **noxtls_tls13_recv_certificate_request**, **noxtls_tls13_recv_certificate**, **noxtls_tls13_recv_certificate_verify**, **noxtls_tls13_recv_finished**
- **noxtls_tls13_send_client_certificate**, **noxtls_tls13_send_client_certificate_verify**, **noxtls_tls13_send_finished**
