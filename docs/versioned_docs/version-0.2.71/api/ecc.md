...
### `noxtls_ecdh_compute_shared_secret_ex`

```c
noxtls_return_t noxtls_ecdh_compute_shared_secret_ex(
    ecc_key_t *private_key, const ecc_point_t *peer_public_key,
    uint8_t *shared_secret, uint32_t *shared_secret_len,
    noxtls_ecdh_diagnostic_t *diagnostic);
```

Diagnostic form of ECDH shared-secret computation. Pass `NULL` for
`diagnostic` when no additional detail is needed. On failure, the optional
`noxtls_ecdh_diagnostic_t` reports a non-secret stage such as invalid peer
public key, scalar multiplication failure, or shared-point infinity, plus the
internal return code. It never contains scalar, point-coordinate, or
shared-secret data.
